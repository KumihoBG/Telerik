...
// използваме автомат - вървим и проверяваме символ по символ и изменяме състоянието в зависимост от това
namespace _06.FakeTextMarkupLanguage
{
    class FakeTextML
    {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                while (currentSymbolIndex < currentLine.Length)
                {
                    if (currentLine[currentSymbolIndex] == '<')
                    {
                        ...
                        int tagLastInd = currentSymbolIndex + tag.Length - 1;
                        currentSymbolIndex = tagLastInd;
                    }
                    else
                    {
                        if (openedDelTags == 0)
